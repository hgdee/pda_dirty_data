...
#pivot_candy_group %>%
#  arrange(desc(customer_satisfaction), desc(candy_bar_num)) %>%
#  summarise(customer_satisfaction, candy_bar_name, candy_bar_num)
...
country_candy <- pivot_candy %>%
        mutate(which_country_do_you_live_in =  
          case_when(which_country_do_you_live_in  
        ...
        group_by(which_country_do_you_live_in,  candy_bar_name) 
...
country_candy %>%
  summarise( sum = sum(case_when(customer_satisfaction == "MEH" ~ 0,
     ...
     top_n(1,sum) %>%
  arrange(which_country_do_you_live_in, desc(sum), .by_group = TRUE)  
